...
const ENTRY_TYPES: Record<
	string,
	{ id: string; label: string; desc: string; payout: string }[]
> = {
	PrizePicks: [
		{
			id: "power",
			label: "Power Play",
			desc: "2-6 picks, all must hit",
			payout: "3x–25x",
		},
		{
			id: "flex",
			label: "Flex Play",
			desc: "3-6 picks, miss 1 still win",
			payout: "1.5x–5x",
		},
	],
	Underdog: [
		{
			id: "flex",
			label: "Flex Play",
			desc: "Miss 1, still win reduced",
			payout: "0.5x–20x",
		},
		{
			id: "power",
			label: "Power Play",
			desc: "All must hit for max payout",
			payout: "3x–20x",
		},
	],
	Sleeper: [
		{
			id: "standard",
			label: "Standard",
			desc: "Pick 2-6, all must hit",
			payout: "3x–25x",
		},
		{
			id: "insured",
			label: "Insured",
			desc: "Miss 1, still get paid",
			payout: "1x–10x",
		},
	],
	"DraftKings Pick6": [
		{
			id: "standard",
			label: "Standard",
			desc: "6 picks, flex scoring",
			payout: "Variable",
		},
		{
			id: "tier",
			label: "Tier Play",
			desc: "Pick from tiered pool",
			payout: "Variable",
		},
	],
	Kalshi: [
		{
			id: "single",
			label: "Single Market",
			desc: "YES/NO binary outcome",
			payout: "1.1x–10x",
		},
		{
			id: "multi",
			label: "Multi-Market",
			desc: "Combine 2+ markets",
			payout: "2x–50x",
		},
	],
};
...
const QUICK_PACKS = [
	{
		id: "bestEdge4",
		icon: "🎯",
		label: "Best Edge 4-Pack",
		desc: "Top absolute edge plays",
	},
	{
		id: "highConfidence3",
		icon: "🛡️",
		label: "High Confidence 3",
		desc: "Safest high-confidence picks",
	},
	{
		id: "balancedMix",
		icon: "⚖️",
		label: "Balanced EV Mix",
		desc: "2 overs + 2 unders",
	},
	{
		id: "topValue5",
		icon: "💎",
		label: "Top Value 5",
		desc: "Best composite Value Score",
	},
	{
		id: "prizePicksPower",
		icon: "⚡",
		label: "PP Power Play",
		desc: "4-pick power play",
	},
	{
		id: "underdogFlex",
		icon: "🐕",
		label: "UD 6-Pick Flex",
		desc: "Underdog flex optimized",
	},
	{
		id: "kalshiBest",
		icon: "💹",
		label: "Kalshi Best",
		desc: "Top Kalshi markets",
	},
];
...
export function PickBuilderPage() {
		...
		switch (platform) {
			...
			case "Kalshi":
				...
				text += myPicks
					.map(
						(p: any) =>
							`${p.playerName} ${p.statType} — ${p.edge > 0 ? "YES" : "NO"} (Line: ${p.line}, Edge: ${p.edge > 0 ? "+" : ""}${p.edge}%)`,
					)
					.join("\n");
				break;
			default:
				text = myPicks
					.map(
						(p: any) =>
							`${p.playerName} ${p.overUnder.toUpperCase()} ${p.line} ${p.statType}`,
					)
					.join("\n");
		}
	...
	const totalEdge =
		myPicks && myPicks.length > 0
			? Math.round(
					(myPicks.reduce(
						(sum: number, p: any) => sum + Math.abs(getAdjustedEdge(p) || 0),
						0,
					) /
						myPicks.length) *
						10,
	...
	const divColor =
		divScore >= 70 ? "#00FF88" : divScore >= 40 ? "#FFB800" : "#FF4466";
	const divLabel =
		divScore >= 70
			? "Well Diversified"
			: divScore >= 40
			...
			<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
				<div>
					...
					<p className="text-sm text-muted-foreground mt-1">
						Build optimized entries with smart correlation detection & one-click
						quick packs
					</p>
				</div>
			</div>
					...
					<div className="mt-3 bg-[#0A0E17] rounded-lg border border-[#A855F7]/20 p-4">
						<div className="flex items-center justify-between mb-3">
							...
							<div className="flex items-center gap-3">
								...
								<div className="text-right">
									...
									<div className="text-sm font-mono font-bold text-[#00D4FF]">
										{quickPack.avgConfidence}%
									</div>
								</div>
								<Button
									size="sm"
						...
						<div className="grid sm:grid-cols-2 gap-2">
							{quickPack.picks.map((p: any) => (
								<div
									key={p._id}
									className="flex items-center justify-between p-2 rounded-lg bg-[#111827] border border-[#1E293B]"
								>
									...
									<div className="flex items-center gap-2 shrink-0">
										...
										<Button
											size="sm"
											variant="ghost"
											onClick={() => handleAddFromPack(p._id)}
											className="size-6 p-0 hover:bg-[#00FF88]/10 hover:text-[#00FF88]"
										>
						...
						<Button
							...
							onClick={async () => {
								...
								toast.success(
									`Added ${quickPack.picks.length} picks from ${quickPack.name}!`,
								);
							...
							{PLATFORM_OPTIONS.map((p) => (
								<button
									...
									onClick={() => {
										...
										setEntryType(ENTRY_TYPES[p]?.[0]?.id || "standard");
									}}
						...
						<div className="grid grid-cols-2 gap-2">
							{platformEntryTypes.map((type) => (
								...
								>
									...
									<div className="text-xs font-mono text-[#A855F7] mt-1">
										{type.payout}
									</div>
								</button>
							))}
						...
						{!myPicks || myPicks.length === 0 ? (
							<div className="p-8 text-center">
								...
								<p className="text-xs text-muted-foreground/60 mt-1">
									Use a <span className="text-[#A855F7]">Quick Pack</span> above
									or go to the{" "}
									<Link to="/props" className="text-[#00D4FF] hover:underline">
										Prop Analyzer
									</Link>{" "}
									to add picks
								</p>
							...
							<div className="divide-y divide-[#1E293B]/50">
								{myPicks.map((pick: any, idx: number) => {
									...
									return (
										<div
											key={pick._id}
											className="flex items-center gap-3 p-3 hover:bg-[#1A2236]/30 transition-colors"
										>
											...
											<div className="flex-1 min-w-0">
												<div className="flex items-center gap-2">
													...
													<span className="text-[10px] text-muted-foreground bg-[#0A0E17] px-1.5 py-0.5 rounded">
														{pick.sport}
													</span>
												</div>
												<div className="text-xs text-muted-foreground mt-0.5">
												...
												<div className="flex items-center gap-2 mt-1">
													...
													<span
														className={`text-xs font-mono font-bold ${adj !== 0 ? "text-[#FFB800]" : "text-white"}`}
													>
											...
											<div className="text-right shrink-0">
												<div
													className={`text-sm font-mono font-bold ${adjustedEdge > 0 ? "text-[#00FF88]" : "text-[#FF4466]"}`}
												>
													{adjustedEdge > 0 ? "+" : ""}
													{adjustedEdge}%
												</div>
												...
												{adj !== 0 && (
													<div className="text-[9px] text-[#FFB800]">
														adj. EV
													</div>
												)}
											</div>
							...
							<div className="p-3 border-t border-[#1E293B] bg-[#0A0E17]/50">
								<div className="flex items-center gap-2 flex-wrap">
									...
									<Button
										variant="ghost"
										size="sm"
										onClick={() => handleCopyForPlatform("PrizePicks")}
										className="text-[10px] h-6 px-2 text-[#00FF88] hover:bg-[#00FF88]/10"
									>
									...
									<Button
										variant="ghost"
										size="sm"
										onClick={() => handleCopyForPlatform("Underdog")}
										className="text-[10px] h-6 px-2 text-[#00D4FF] hover:bg-[#00D4FF]/10"
									>
									...
									<Button
										variant="ghost"
										size="sm"
										onClick={() => handleCopyForPlatform("Kalshi")}
										className="text-[10px] h-6 px-2 text-[#A855F7] hover:bg-[#A855F7]/10"
									>
									...
									<Button
										...
										className="text-[10px] h-6 px-2 text-[#FFB800] hover:bg-[#FFB800]/10"
									>
								...
								{correlations.warnings?.length > 0 && (
									<div className="bg-[#FF4466]/5 border border-[#FF4466]/20 rounded-xl p-4">
										<div className="flex items-center gap-2 mb-2">
											...
											<span className="text-sm font-semibold text-[#FF4466]">
												Correlation Warnings
											</span>
										</div>
										{correlations.warnings.map((w: string, i: number) => (
											<p key={i} className="text-xs text-[#FF4466]/80 mb-1">
												{w}
											</p>
										))}
									</div>
								...
								{correlations.suggestions?.length > 0 && (
									<div className="bg-[#00D4FF]/5 border border-[#00D4FF]/20 rounded-xl p-4">
										...
										{correlations.suggestions.map((s: string, i: number) => (
											<p key={i} className="text-xs text-[#00D4FF]/80 mb-1">
												{s}
											</p>
										))}
									</div>
							...
							<div className="bg-[#00FF88]/5 border border-[#00FF88]/20 rounded-xl p-4">
								<div className="flex items-center gap-2 mb-3">
									...
									<span className="text-sm font-semibold text-[#00FF88]">
										Suggested Picks to Diversify
									</span>
									<span className="text-[10px] text-muted-foreground">
										AI-selected to improve balance
									</span>
								</div>
								<div className="grid sm:grid-cols-2 gap-2">
					...
					<div className="bg-[#111827] rounded-xl border border-[#1E293B] p-5 sticky top-4">
							...
							<div className="mb-4 p-3 rounded-lg bg-[#0A0E17] border border-[#1E293B]">
								...
								<div className="w-full h-2 bg-[#1E293B] rounded-full overflow-hidden">
									<div
										...
										style={{ width: `${divScore}%`, backgroundColor: divColor }}
									/>
								</div>
						...
						<div className="space-y-3 mb-5 py-4 border-t border-[#1E293B]">
							...
							<div className="flex justify-between text-sm">
								...
								<span
									className={`font-mono font-bold ${totalEdge > 0 ? "text-[#00FF88]" : "text-[#FF4466]"}`}
								>
									{totalEdge > 0 ? "+" : ""}
									{totalEdge}%
								</span>
							...
							<div className="flex justify-between text-sm">
								...
								<span className="text-white font-mono">
									{myPicks
										? [...new Set(myPicks.map((p: any) => p.sport))].length
										: 0}
								</span>
							</div>
							<div className="flex justify-between text-sm">
								...
								<span className="text-white font-mono">
									{myPicks?.filter((p: any) => p.overUnder === "over").length ||
										0}{" "}
									/{" "}
									{myPicks?.filter((p: any) => p.overUnder === "under")
										.length || 0}
								</span>
							</div>
							<div className="flex justify-between text-sm">
