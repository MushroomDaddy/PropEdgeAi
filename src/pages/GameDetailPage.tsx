		...
		<div className="flex-1 overflow-auto p-4 md:p-6 space-y-6">
			...
			<div className="flex gap-1 rounded-lg border border-border/50 bg-card/30 p-1">
				{[
					...
					{
						key: "props" as const,
						label: `Props (${gameProps?.length ?? 0})`,
						icon: BarChart3,
					},
					...
					<div className="divide-y divide-border/20">
						{quarters[q].map((play, i) => {
							...
							return (
								<div
									key={i}
									className={`flex items-start gap-3 px-4 py-3 ${isScore ? "bg-primary/5" : ""}`}
								>
									{/* Time */}
									...
									{/* Team indicator */}
									<span
										className={`text-xs font-bold w-10 shrink-0 pt-0.5 ${isHome ? "text-green-400" : "text-blue-400"}`}
									>
									...
									<div className="flex-1 min-w-0">
										<span
											className={`text-sm ${isScore ? "font-semibold text-foreground" : isTurnover ? "text-red-400" : isFoul ? "text-yellow-400" : isTimeout ? "text-muted-foreground italic" : "text-foreground/80"}`}
										>
											{play.description}
										</span>
									</div>
...
function BoxScoreTab({
	...
	boxScore?: { home: BoxScorePlayer[]; away: BoxScorePlayer[] };
	homeTeam: string;
	awayTeam: string;
}) {
	if (!boxScore) {
				...
				<table className="w-full text-xs">
					<thead>
						<tr className="border-b border-border/30 text-muted-foreground">
							<th className="text-left px-3 py-2 font-medium sticky left-0 bg-card/80">
								Player
							</th>
					...
					<tbody>
						{players.map((p, i) => (
							...
							>
								...
								<td
									className={`px-2 py-2 text-center font-bold ${p.points >= 20 ? "text-primary" : ""}`}
								>
									{p.points}
								</td>
								<td
									className={`px-2 py-2 text-center ${p.rebounds >= 8 ? "text-blue-400" : ""}`}
								>
									{p.rebounds}
								</td>
								<td
									className={`px-2 py-2 text-center ${p.assists >= 5 ? "text-purple-400" : ""}`}
								>
									{p.assists}
								</td>
								<td className="px-2 py-2 text-center">{p.steals}</td>
								<td className="px-2 py-2 text-center">{p.blocks}</td>
								<td className="px-2 py-2 text-center text-red-400/70">
									{p.turnovers}
								</td>
								<td className="px-2 py-2 text-center text-muted-foreground">
									{p.fg}
								...
								<td
									className={`px-2 py-2 text-center font-medium ${p.plusMinus > 0 ? "text-green-400" : p.plusMinus < 0 ? "text-red-400" : ""}`}
								>
								...
								>
									<div className="flex-1 min-w-0">
										...
										<div className="text-xs text-muted-foreground">
											{p.statType as string} • {p.platform as string}
										</div>
									</div>
...
function RosterTab({
	...
	roster?: {
		home: { active: string[]; out: OutPlayer[] };
		away: { active: string[]; out: OutPlayer[] };
	};
	homeTeam: string;
	awayTeam: string;
}) {
...
function TeamRoster({
	...
	teamName: string;
	roster: { active: string[]; out: OutPlayer[] };
	label: string;
}) {
	return (
		<div className="rounded-xl border border-border/50 bg-card/30 overflow-hidden">
			...
			<div className="p-4 space-y-3">
				...
				<div className="flex flex-wrap gap-2">
					{roster.active.map((name, i) => (
						<span
							key={i}
							className="rounded-lg bg-muted/30 border border-border/30 px-3 py-1.5 text-xs font-medium"
						>
							{name}
						</span>
					))}
				...
				<div className="p-4 pt-0 space-y-2">
					<div className="flex items-center gap-2 mb-2">
						...
						<span className="text-xs font-medium text-red-400">
							OUT / INACTIVE ({roster.out.length})
						</span>
					</div>
					{roster.out.map((p, i) => (
						<div
