...
const PRESETS = [
  ...
  { icon: "🏈", label: "NFL Props", query: "Best NFL player props with edge and consensus analysis" },
  ...
  { icon: "🐕", label: "Underdog Entry", query: "Build me an optimized 5-pick Underdog flex entry under $25" },
  { icon: "💎", label: "Value Score", query: "Show the highest Value Score picks across all sports and platforms" },
  { icon: "📊", label: "Full Analysis", query: "Give me a complete analysis of tonight's slate — edges, games, and picks" },
];
    ...
    <div className="flex flex-col h-[calc(100vh-4rem)] max-w-[1200px]">
      ...
      <div className="flex items-center justify-between pb-4 border-b border-[#1E293B]">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-white">PropEdge AI Analyst</h1>
            <p className="text-xs text-muted-foreground">Step-by-step reasoning with real data citations</p>
          </div>
        </div>
      ...
      {(!chatMessages || chatMessages.length === 0) && (
        <div className="py-6">
          <div className="text-center mb-4">
            ...
            <h2 className="text-lg font-bold text-white">What can I analyze?</h2>
            <p className="text-sm text-muted-foreground mt-1">Pick a preset or ask anything about today's props, edges, and markets</p>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2">
            {PRESETS.map((preset, i) => (
              ...
              >
                ...
                <div className="text-xs font-semibold text-white group-hover:text-[#A855F7] transition-colors">{preset.label}</div>
              </button>
            ))}
          </div>
        </div>
            ...
            <div className="bg-[#111827] border border-[#1E293B] rounded-xl p-4 max-w-[80%]">
              <div className="flex items-center gap-2">
                <div className="flex gap-1">
                  <div className="size-2 rounded-full bg-[#A855F7] animate-bounce" style={{ animationDelay: "0ms" }} />
                  <div className="size-2 rounded-full bg-[#00FF88] animate-bounce" style={{ animationDelay: "150ms" }} />
                  <div className="size-2 rounded-full bg-[#00D4FF] animate-bounce" style={{ animationDelay: "300ms" }} />
                </div>
    ...
    <div className="text-sm text-[#C8D0E0] space-y-1.5">
      {lines.map((line, i) => {
        ...
        if (trimmed.startsWith("## ")) {
          return <h2 key={i} className="text-base font-bold text-white mt-3 mb-1">{trimmed.slice(3)}</h2>;
        }
        // ### Sub-header
        if (trimmed.startsWith("### ")) {
          return <h3 key={i} className="text-sm font-bold text-[#00D4FF] mt-2 mb-1">{trimmed.slice(4)}</h3>;
        }
        // Step markers
          ...
          return (
            <div key={i} className="flex items-center gap-2 mt-2 mb-1">
              <div className="size-5 rounded-full bg-[#A855F7]/20 flex items-center justify-center shrink-0">
                <span className="text-[9px] font-bold text-[#A855F7]">{trimmed.match(/Step (\d)/)?.[1] || "?"}</span>
              </div>
              <span className="text-xs font-semibold text-[#A855F7]">{trimmed.replace(/\*/g, "")}</span>
            </div>
          );
        }
        // Horizontal rule
        ...
        if (trimmed.startsWith("• ") || trimmed.startsWith("- ")) {
          return (
            <div key={i} className="flex items-start gap-2 pl-1">
              ...
              <span className="text-sm"><InlineFormatted text={trimmed.slice(2)} /></span>
            </div>
          );
        }
        // Numbered
        if (/^\d+\./.test(trimmed)) {
          return (
            <div key={i} className="flex items-start gap-2 pl-1">
              <span className="text-[#00D4FF] font-mono text-xs mt-0.5 shrink-0">{trimmed.match(/^(\d+\.)/)?.[1]}</span>
              <span className="text-sm"><InlineFormatted text={trimmed.replace(/^\d+\.\s*/, "")} /></span>
            </div>
          );
        }
        // Default paragraph
