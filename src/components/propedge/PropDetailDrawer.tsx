        ...
        <div className="sticky top-0 bg-card/95 backdrop-blur-sm border-b border-white/10 px-5 py-4 flex items-center justify-between z-10">
          <div>
            <h3 className="text-sm font-semibold">
              {prop.playerName || "Prop"} — {formatLabel(prop.statType)}
            </h3>
          ...
          <button
            onClick={onClose}
            className="size-8 rounded-lg border border-white/10 flex items-center justify-center hover:bg-white/5 transition-colors"
          >
            <X className="size-4" />
          </button>
        </div>
        ...
        <div className="p-5 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="rounded-xl bg-white/5 border border-white/5 p-3 text-center">
              ...
              <div className="flex justify-center">
                {prop.valueScore !== undefined ? (
                  <ValueScoreBadge score={prop.valueScore} size="md" />
                ) : (
          ...
          <div className="rounded-xl border border-white/5 bg-white/[0.02] p-4">
            ...
            <ul className="space-y-1.5 text-xs text-muted-foreground">
              {prop.edge > 5 && (
                <li>
                  • Strong edge of {prop.edge.toFixed(1)}% above market value
                </li>
              )}
              {prop.modelProb && prop.modelProb > 60 && (
                <li>
                  • Model probability of {prop.modelProb}% indicates high
                  confidence
                </li>
              )}
              {projDiff !== 0 && (
                <li>
                  • Projection ({prop.projection}) is{" "}
                  {Math.abs(projDiff).toFixed(1)} {isOver ? "above" : "below"}{" "}
                  the line ({prop.line})
                </li>
              )}
              {prop.confidence && prop.confidence >= 65 && (
                <li>
                  • Confidence score of {prop.confidence}% from multiple data
                  sources
                </li>
              )}
              {prop.hitRate && prop.hitRate > 55 && (
                <li>
                  • Historical hit rate of {prop.hitRate}% on similar lines
                </li>
              )}
            </ul>
          </div>
          ...
          <div className="rounded-xl border border-white/5 bg-white/[0.02] p-4">
            ...
            <ul className="space-y-1.5 text-xs text-muted-foreground">
              {prop.bustRisk !== undefined && prop.bustRisk > 30 && (
                <li>• Bust risk at {prop.bustRisk}%</li>
              )}
              {prop.edge < 3 && (
                <li>• Thin edge — small line movement could eliminate value</li>
              )}
              {prop.confidence && prop.confidence < 55 && (
                <li>• Lower confidence score — fewer data sources agree</li>
              )}
              <li>
                • <DataSourceBadge source="demo" /> Mock data — verify with live
                sources before wagering
              </li>
            </ul>
          </div>
          ...
          <div className="rounded-xl border border-white/5 bg-white/[0.02] p-4">
            ...
            <div className="grid grid-cols-2 gap-y-2.5 gap-x-4 text-xs">
              ...
              <StatRow
                label="Proj Diff"
                value={`${projDiff > 0 ? "+" : ""}${projDiff.toFixed(1)}`}
                color={projDiff > 0 ? "text-emerald-400" : "text-red-400"}
              />
              <StatRow
                label="Edge"
                value={`${prop.edge > 0 ? "+" : ""}${prop.edge.toFixed(1)}%`}
                color={prop.edge > 0 ? "text-emerald-400" : "text-red-400"}
              />
              {prop.modelProb !== undefined && (
                <StatRow label="Model Prob" value={`${prop.modelProb}%`} />
              )}
              {prop.marketImpliedProb !== undefined && (
                <StatRow
                  label="Market Implied"
                  value={`${prop.marketImpliedProb}%`}
                />
              )}
              {prop.confidence !== undefined && (
                <StatRow label="Confidence" value={`${prop.confidence}%`} />
              )}
              {prop.bustRisk !== undefined && (
                <StatRow
                  label="Bust Risk"
                  value={`${prop.bustRisk}%`}
                  color={prop.bustRisk > 40 ? "text-red-400" : undefined}
                />
              )}
            </div>
          </div>
