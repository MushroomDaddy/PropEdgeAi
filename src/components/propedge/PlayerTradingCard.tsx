...
import { getTeamColors, getPlayerInitials, getSportIcon } from "../../lib/assets";
...
export function PlayerTradingCard({
  playerName, team, position, sport, headshotUrl,
  stats = [], edge, confidence, onClick, compact = false,
}: Props) {
    ...
    >
      ...
      <div className="relative bg-[#0A0E17] rounded-2xl border border-white/10 overflow-hidden">
        ...
        >
          ...
          <span className="absolute top-2 right-3 text-2xl opacity-20">{sportIcon}</span>
        ...
        <div className="pt-10 px-4 pb-4 text-center space-y-3">
          <div>
            ...
            <div className="text-[10px] text-muted-foreground flex items-center justify-center gap-1.5 mt-0.5">
              <span>{team}</span>
              {position && <><span className="opacity-30">•</span><span>{position}</span></>}
            </div>
          </div>
            ...
            <div className="flex items-center justify-center gap-2">
              {edge !== undefined && (
                <div className={`flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold ${
                  edge > 0 ? "bg-emerald-400/10 text-emerald-400" : "bg-red-400/10 text-red-400"
                }`}>
          ...
          <div className="flex items-center justify-center gap-1 pt-1">
            ...
            <span className="text-[8px] text-muted-foreground tracking-wider uppercase">PropEdge AI</span>
          </div>
        </div>
      </div>
    </motion.div>
