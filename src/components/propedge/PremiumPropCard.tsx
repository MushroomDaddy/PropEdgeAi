...
export function PremiumPropCard(props: PremiumPropCardProps) {
  const {
    playerName, team, teamColor, teamLogoUrl, imageUrl,
    statType, line, projection, edge, overUnder, confidence,
    bustRisk, hitRate, platform, modelProb, onClick,
  } = props;
  ...
  return (
    ...
    >
      ...
      <div className="h-1 w-full" style={{ backgroundColor: teamColor || "#333" }} />
      ...
      <div className="p-4 space-y-3">
        ...
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-3">
            {imageUrl ? (
              <img src={imageUrl} alt={playerName} className="size-10 rounded-full object-cover ring-2 ring-white/10" />
            ) : (
          ...
          <span className={`text-xs font-bold px-2 py-0.5 rounded ${dirColor} ${overUnder === "over" ? "bg-emerald-400/10" : "bg-red-400/10"}`}>
            {dirLabel}
          </span>
        </div>
        ...
        <div className="flex items-baseline justify-between">
          ...
          {modelProb !== undefined && (
            <div className="text-right">
              ...
              <div className="text-lg font-bold font-mono text-[#00D4FF]">{modelProb}%</div>
            </div>
          )}
        </div>
