...
export function StatTrendCard({
  ...
  if (l5 === undefined && l10 === undefined && season === undefined)
    return null;
    ...
    <div className="rounded-xl border border-white/5 bg-card/50 p-4 hover:border-white/10 transition-colors">
      ...
      <div className="flex items-end gap-1 h-8 mb-3">
        {[season, l10, l5].map((val, i) => {
          if (val === undefined)
            return <div key={i} className="flex-1 rounded-sm bg-white/5 h-2" />;
          ...
          return (
            <div key={i} className="flex-1 flex flex-col items-center gap-0.5">
              <div
                className={cn("w-full rounded-sm transition-all", colors[i])}
                style={{ height: `${height}%` }}
              />
            </div>
          );
