...
export function PropsAnalyzerPage() {
	...
	const filteredProps = useMemo(() => {
		...
		if (propType !== "All") {
			filtered = filtered.filter(
				(p: any) => (p.propType || "over_under") === propType,
			);
		}
		filtered.sort((a: any, b: any) => {
			const aVal =
				sortKey === "edge" ? Math.abs(a[sortKey] || 0) : a[sortKey] || 0;
			const bVal =
				sortKey === "edge" ? Math.abs(b[sortKey] || 0) : b[sortKey] || 0;
		...
		<div className="space-y-5 max-w-[1500px]">
			...
			<div>
				...
				<p className="text-sm text-muted-foreground mt-1">
					Search, filter, and analyze player props with deep statistical
					insights · Edge = Model Prob − Market Implied Prob
				</p>
			</div>
							...
							{filteredProps.map((prop: any) => {
								...
								return (
									<tr
										...
										onClick={() =>
											setSelectedProp(
												selectedProp?._id === prop._id ? null : prop,
											)
										}
									>
										<td className="p-3">
										...
										<td className="p-3 text-right">
											<ConfidenceBadge value={prop.confidence} />
										</td>
										<td className="p-3 text-center">
											{consensus ? (
												<ConsensusIndicator
													numOverLine={consensus.numOverLine}
													numSources={consensus.numSources}
												/>
											) : (
										...
										<td className="p-3 text-right">
											<BustRiskBadge value={prop.bustRisk} />
										</td>
										<td className="p-3 text-center">
											<Badge
										...
										<td className="p-3">
											<span
												className={`text-xs px-2 py-0.5 rounded whitespace-nowrap ${
													prop.platform === "Kalshi"
														? "text-[#A855F7] bg-[#A855F7]/10"
														: "text-muted-foreground bg-[#1A2236]"
												}`}
											>
			...
			{selectedProp && (
				<div className="bg-[#111827] rounded-xl border border-[#00D4FF]/20 p-5">
					<div className="flex items-start justify-between mb-4">
						<div>
							<h3 className="text-lg font-bold text-white">
								{selectedProp.playerName} — {formatLabel(selectedProp.statType)}
							</h3>
							<p className="text-sm text-muted-foreground">
								{selectedProp.team} · {selectedProp.sport} ·{" "}
								{formatLabel(selectedProp.platform)}
							</p>
						</div>
						<div className="flex items-center gap-3">
							<ValueScoreBadge score={selectedProp.valueScore} size="lg" />
							<Button
								onClick={() => handleAddPick(selectedProp._id)}
								className="bg-[#00FF88] hover:bg-[#00FF88]/90 text-[#0A0E17] font-bold"
								size="sm"
							>
					...
					<div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-7 gap-3 mb-4">
						...
						<MiniStat
							label="Model Prob"
							value={`${selectedProp.modelProb ?? "—"}%`}
							color="#A855F7"
						/>
						<MiniStat
							label="Mkt Implied"
							value={`${selectedProp.marketImpliedProb ?? "—"}%`}
							color="#FFB800"
						/>
						<MiniStat
							label="Edge (M−I)"
							value={`${selectedProp.edge > 0 ? "+" : ""}${selectedProp.edge}%`}
							color={selectedProp.edge > 0 ? "#00FF88" : "#FF4466"}
						/>
						<MiniStat
							label="Confidence"
							value={`${selectedProp.confidence}%`}
							color="#00D4FF"
						/>
						<MiniStat
							label="Hit Rate"
							value={`${selectedProp.hitRate}%`}
							color="#FFB800"
						/>
						<MiniStat
							label="Bust Risk"
							value={`${selectedProp.bustRisk ?? "?"}%`}
							color={
								(selectedProp.bustRisk || 40) <= 30
									? "#00FF88"
									: (selectedProp.bustRisk || 40) <= 55
										? "#FFB800"
										: "#FF4466"
							}
						/>
						...
						<div className="bg-[#0A0E17] rounded-lg border border-[#1E293B] p-4 mb-4">
							...
							<div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-3">
								<div>
									...
									<div
										className={`text-lg font-bold font-mono ${selectedProp.projectionConsensus.avg > selectedProp.line ? "text-[#00FF88]" : "text-[#FF4466]"}`}
									>
								...
								<div>
									...
									<div className="text-lg font-bold font-mono text-white">
										{selectedProp.projectionConsensus.numOverLine}/
										{selectedProp.projectionConsensus.numSources}
									</div>
								</div>
								<div>
									<div className="text-xs text-muted-foreground">
									...
									<div className="text-lg font-bold font-mono text-[#FFB800]">
										±{selectedProp.projectionConsensus.spread}
									</div>
								</div>
								<div>
							...
							<div className="relative h-3 bg-[#1E293B] rounded-full overflow-hidden">
								<div
									...
									style={{
										...
										backgroundColor:
											selectedProp.projectionConsensus.numOverLine >
											selectedProp.projectionConsensus.numSources / 2
												? "#00FF88"
												: "#FF4466",
									}}
								/>
						...
						<div className="bg-[#0A0E17] rounded-lg border border-[#A855F7]/20 p-4 mb-4">
							<div className="flex items-center gap-2 mb-3">
								...
								<span className="text-[10px] text-muted-foreground">
									({selectedProp.monteCarloSim.simulations.toLocaleString()}{" "}
									sims)
								</span>
							</div>
							<div className="grid grid-cols-2 sm:grid-cols-5 gap-3 mb-3">
								<div>
									...
									<div
										className={`text-lg font-bold font-mono ${selectedProp.monteCarloSim.hitRate >= 55 ? "text-[#00FF88]" : selectedProp.monteCarloSim.hitRate >= 45 ? "text-[#FFB800]" : "text-[#FF4466]"}`}
									>
								...
								<div>
									...
									<div className="text-lg font-bold font-mono text-[#FF4466]">
										{selectedProp.monteCarloSim.p10}
									</div>
								</div>
								<div>
									<div className="text-xs text-muted-foreground">
									...
									<div className="text-lg font-bold font-mono text-white">
										{selectedProp.monteCarloSim.p50}
									</div>
								</div>
								<div>
									<div className="text-xs text-muted-foreground">
									...
									<div className="text-lg font-bold font-mono text-[#00FF88]">
										{selectedProp.monteCarloSim.p90}
									</div>
								</div>
								<div>
									<div className="text-xs text-muted-foreground">Std Dev</div>
									<div className="text-lg font-bold font-mono text-[#FFB800]">
										{selectedProp.monteCarloSim.stdDev}
									</div>
								</div>
							</div>
							...
							<div className="flex items-center justify-between mt-2 text-[10px]">
								<span className="text-[#FF4466]">
									■ Below line ({100 - selectedProp.monteCarloSim.hitRate}%)
								</span>
								<span className="text-[#FFB800]">
									| Line: {selectedProp.line}
								</span>
								<span className="text-[#00FF88]">
									■ Above line ({selectedProp.monteCarloSim.hitRate}%)
								</span>
							</div>
						</div>
					)}
							...
							<div className="grid sm:grid-cols-3 gap-3 mb-3">
								<div className="bg-[#111827] rounded-lg p-3 border border-[#1E293B]">
									...
									<div className="text-2xl font-bold font-mono text-[#00D4FF]">
										{selectedProp.historicalHitRate.similarLines}%
									</div>
									<div className="text-[10px] text-muted-foreground">
										{selectedProp.historicalHitRate.sampleSize} game sample
									</div>
								</div>
								...
								<div className="bg-[#111827] rounded-lg p-3 border border-[#1E293B]">
									...
									<div className="text-2xl font-bold font-mono text-white">
										{selectedProp.last10Hits || "?"}/10
									</div>
									<div className="text-[10px] text-muted-foreground">
										Trend:{" "}
										{selectedProp.last10Trend === "up"
											? "🔺 Improving"
											: selectedProp.last10Trend === "down"
												? "🔻 Declining"
												: "➡️ Stable"}
									</div>
								</div>
								...
								<div className="flex-1">
									...
									<div className="h-2 bg-[#1E293B] rounded-full overflow-hidden">
										<div
											...
											style={{
												width: `${selectedProp.historicalHitRate.similarLines}%`,
											}}
										/>
									</div>
								</div>
									...
									<div className="flex-1">
										...
										<div className="h-2 bg-[#1E293B] rounded-full overflow-hidden">
											<div
												...
												style={{
													width: `${selectedProp.historicalHitRate.vsTeam}%`,
												}}
											/>
										</div>
									</div>
					...
					<div className="grid sm:grid-cols-3 gap-3 mb-4">
						{selectedProp.hotColdStreak && (
							<div
								className={`bg-[#0A0E17] rounded-lg border p-3 ${
									selectedProp.hotColdStreak.type === "hot"
										? "border-[#FF6B35]/30"
										: selectedProp.hotColdStreak.type === "cold"
											? "border-[#00D4FF]/30"
											: "border-[#1E293B]"
								}`}
							>
								...
								<div className="text-xl font-bold">
									{selectedProp.hotColdStreak.label}
								</div>
								<div className="text-[10px] text-muted-foreground">
									{selectedProp.hotColdStreak.type === "hot"
										? "Player is outperforming projections"
										: selectedProp.hotColdStreak.type === "cold"
											? "Player is underperforming recently"
						...
						{selectedProp.bustRisk != null && (
							<div className="bg-[#0A0E17] rounded-lg border border-[#1E293B] p-3">
								...
								<div
									className={`text-xl font-bold font-mono ${selectedProp.bustRisk <= 30 ? "text-[#00FF88]" : selectedProp.bustRisk <= 55 ? "text-[#FFB800]" : "text-[#FF4466]"}`}
								>
								...
								<div className="w-full h-1.5 bg-[#1E293B] rounded-full mt-1 overflow-hidden">
									<div
										...
										style={{
											...
											backgroundColor:
												selectedProp.bustRisk <= 30
													? "#00FF88"
													: selectedProp.bustRisk <= 55
														? "#FFB800"
														: "#FF4466",
										}}
									/>
								...
								<div className="text-[10px] text-muted-foreground mt-1">
									{selectedProp.bustRisk <= 30
										? "Low bust probability"
										: selectedProp.bustRisk <= 55
											? "Moderate bust risk"
											: "High bust risk — tread carefully"}
								</div>
							</div>
						)}
						<div className="bg-[#0A0E17] rounded-lg border border-[#1E293B] p-3">
							...
							<div className="text-xl font-bold font-mono text-white">
								{selectedProp.matchupRating}/10
							</div>
							<div className="text-[10px] text-muted-foreground">
								DvP Rank: #{selectedProp.dvpRank || "?"}
							</div>
						</div>
					</div>
					...
					<div className="mb-4">
						<h4 className="text-sm font-semibold text-white mb-2 flex items-center gap-1">
							<TrendingUp className="size-3.5 text-[#00D4FF]" /> Projection
							Sources
						</h4>
						<div className="grid grid-cols-3 sm:grid-cols-5 gap-2">
							{selectedProp.projectionSources?.map((src: any, i: number) => {
								...
								return (
									<div
										key={i}
										className="bg-[#0A0E17] rounded-lg p-3 border border-[#1E293B]"
									>
										<div className="text-xs text-muted-foreground">
											{src.source}
										</div>
										<div
											className={`text-lg font-bold font-mono ${isOverLine ? "text-[#00FF88]" : "text-[#FF4466]"}`}
										>
											{src.value}
										</div>
									</div>
							...
							<div className="grid grid-cols-2 gap-3">
								<div className="bg-[#0A0E17] rounded-lg p-3 border border-[#1E293B]">
									...
									<div className="text-xl font-bold font-mono text-[#00FF88]">
										{selectedProp.kalshiPayout.yesPayout.toFixed(2)}x
									</div>
								</div>
								<div className="bg-[#0A0E17] rounded-lg p-3 border border-[#1E293B]">
									<div className="text-xs text-[#FF4466]">NO Payout</div>
									<div className="text-xl font-bold font-mono text-[#FF4466]">
										{selectedProp.kalshiPayout.noPayout.toFixed(2)}x
									</div>
								</div>
							</div>
						</div>
					...
					<LineMovementSection
						propId={selectedProp._id}
						playerName={selectedProp.playerName}
					/>

					{/* Correlated Stats */}
					{selectedProp.correlatedWith &&
						selectedProp.correlatedWith.length > 0 && (
							<div>
								<h4 className="text-xs font-semibold text-[#7B8BA8] mb-2">
									Correlated Stats
								</h4>
								<div className="flex flex-wrap gap-2">
									{selectedProp.correlatedWith.map(
										(stat: string, i: number) => (
											<Badge
												key={i}
												className="text-[10px] bg-[#FFB800]/10 text-[#FFB800] border-[#FFB800]/20"
											>
												{stat}
											</Badge>
										),
...
function ValueScoreBadge({
	...
	return (
		<div
			className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded-md ${bg}`}
		>
...
function ConfidenceBadge({ value }: { value: number }) {
	...
	return (
		<span
			className={`text-xs font-mono font-bold px-1.5 py-0.5 rounded ${color}`}
		>
			{value}
		</span>
	);
			...
			<div className="flex gap-0.5">
				{Array.from({ length: numSources }).map((_, i) => (
					<div
						...
						style={{ backgroundColor: i < numOverLine ? color : "#1E293B" }}
					/>
				))}
			</div>
			<span className="text-[10px] font-mono" style={{ color }}>
				{numOverLine}/{numSources}
			</span>
		</div>
	);
}
...
function StreakBadge({ type, label }: { type: string; label: string }) {
	if (type === "hot")
		return (
			<span className="text-[10px] font-bold text-[#FF6B35] bg-[#FF6B35]/10 px-1.5 py-0.5 rounded">
				{label}
			</span>
		);
	if (type === "cold")
	...
	return (
		<div className="bg-[#0A0E17] rounded-lg border border-[#00D4FF]/20 p-4 mb-4">
			<div className="flex items-center justify-between mb-3">
				...
				<a
					href={`/players?q=${encodeURIComponent(playerName)}`}
			...
			<div className="space-y-1">
				{snapshots.map((s: any, i: number) => {
					...
					return (
						<div
							key={i}
							className="flex items-center gap-3 text-xs py-1 border-b border-white/5 last:border-0"
						>
							...
							{lineDelta !== 0 && (
								<span
									className={`font-mono ${lineDelta > 0 ? "text-red-400" : "text-[#00FF88]"}`}
								>
									{lineDelta > 0 ? "▲" : "▼"}
									{Math.abs(lineDelta).toFixed(1)}
								</span>
							)}
							<span className="font-mono text-muted-foreground">
								Proj: {s.projection?.toFixed(1)}
							</span>
							<span
								className={`font-mono ${(s.edge || 0) > 0 ? "text-[#00FF88]" : "text-red-400"}`}
							>
