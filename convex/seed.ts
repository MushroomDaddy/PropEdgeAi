  ...
  handler: async ctx => {
      ...
      NBA: [
        {
          ...
          baseLine: 24,
          variance: 6,
          correlatedWith: ["Pts+Reb+Ast", "Fantasy Points"],
        },
        {
          type: "Rebounds",
          baseLine: 8,
          variance: 3,
          correlatedWith: ["Pts+Reb+Ast"],
        },
        {
          type: "Assists",
          baseLine: 6,
        ...
        },
        {
          ...
          baseLine: 2.5,
          variance: 1.2,
          correlatedWith: ["Points"],
        },
        {
          type: "Pts+Reb+Ast",
          baseLine: 38,
          variance: 8,
          correlatedWith: ["Points", "Rebounds", "Assists"],
        },
        {
          type: "Steals+Blocks",
          baseLine: 2.0,
      ...
      NFL: [
        {
          ...
          baseLine: 265,
          variance: 55,
          correlatedWith: ["Passing TDs", "Fantasy Points"],
        },
        {
          type: "Passing TDs",
          baseLine: 2.0,
          variance: 0.8,
          correlatedWith: ["Passing Yards"],
        },
        {
          type: "Rushing Yards",
          baseLine: 55,
          variance: 22,
          correlatedWith: ["Fantasy Points"],
        },
        {
          type: "Receiving Yards",
          baseLine: 68,
          variance: 25,
          correlatedWith: ["Receptions"],
        },
        {
          type: "Receptions",
          baseLine: 5.5,
          variance: 2.0,
          correlatedWith: ["Receiving Yards"],
        },
        {
          type: "Fantasy Points",
          baseLine: 18,
          variance: 7,
          correlatedWith: ["Passing Yards", "Rushing Yards"],
        },
      ],
      MLB: [
        {
          ...
          baseLine: 2.5,
          variance: 1.5,
          correlatedWith: ["Total Bases"],
        },
        {
          type: "Total Bases",
          baseLine: 2.0,
          variance: 1.3,
          correlatedWith: ["Hits+Runs+RBIs", "Home Runs"],
        },
        {
          type: "Strikeouts (P)",
          baseLine: 6.5,
      ...
      NHL: [
        {
          ...
          baseLine: 3.5,
          variance: 1.5,
          correlatedWith: ["Goals", "Points"],
        },
        {
          type: "Points",
          baseLine: 1.0,
          variance: 0.6,
          correlatedWith: ["Assists", "Goals"],
        },
        {
          type: "Assists",
          baseLine: 0.7,
        ...
        },
        {
          ...
          baseLine: 0.5,
          variance: 0.3,
          correlatedWith: ["Shots on Goal", "Points"],
        },
        {
          type: "Blocked Shots",
          baseLine: 2.0,
    ...
    function findGameForPlayer(team: string, sport: string) {
      const game = allGames.find(
        g => g.sport === sport && (g.homeTeam === team || g.awayTeam === team),
      );
      if (!game) return null;
    ...
    const playerMeta: Record<
      string,
      { jerseyNumber: number; teamColor: string }
    > = {
    ...
    for (const player of allPlayers) {
      ...
      for (let i = 0; i < Math.min(numProps, shuffledStats.length); i++) {
        ...
        const platformIdx = Math.floor(
          pseudoRandom(player.name + stat.type) * platforms.length,
        );
        ...
        const projectionDiff =
          Math.round(((projection - line) / line) * 1000) / 10;
        ...
        const marketImpliedProb = Math.min(
          85,
          Math.max(
            15,
        ...
        const srcAvg =
          Math.round(
            (srcValues.reduce((a, b) => a + b, 0) / srcValues.length) * 10,
          ) / 10;
        const srcSpread =
          Math.round((Math.max(...srcValues) - Math.min(...srcValues)) * 10) /
          10;
        const numOverLine = srcValues.filter(v => v > line).length;

        // R3: Bust risk (0-100)
        const bustRisk = Math.min(
          95,
          Math.max(
            5,
            Math.round(
        ...
        if (player.recentForm === "hot") {
          ...
          streakType =
            streakRand > 0.6 ? "hot" : streakRand > 0.3 ? "neutral" : "cold";
        ...
        const streakLabel =
          streakType === "hot"
            ? `🔥 ${streakGames}G Hot`
            : streakType === "cold"
              ? `❄️ ${streakGames}G Cold`
              : `➡️ Neutral`;

        // R3: Monte Carlo simulation results
        const simHitRate = Math.min(
          92,
          Math.max(
            12,
            Math.round(
    ...
    const firstNBAPlayer = await ctx.db
      .query("players")
    ...
    if (celticsMiamiGame && firstNBAPlayer) {
      const kalshiSpecials = [
        {
          statType: "Team Total Over 110.5",
          line: 110.5,
          projection: 114.2,
          playerName: "Boston Celtics",
          team: "Boston Celtics",
        },
        {
          statType: "Team Total Over 105.5",
          line: 105.5,
          projection: 102.8,
          playerName: "Miami Heat",
          team: "Miami Heat",
        },
        {
          statType: "Game Total Over 218.5",
          line: 218.5,
          projection: 221.4,
          playerName: "BOS vs MIA",
          team: "Boston Celtics",
        },
        {
          statType: "Spread -5.5",
      ...
      for (const km of kalshiSpecials) {
        const kalshiProjDiff =
          Math.round(((km.projection - km.line) / km.line) * 1000) / 10;
        const kalshiZScore = (km.projection - km.line) / 8; // 8 = variance for Kalshi specials
        const kalshiModelProb = Math.min(
          90,
          Math.max(
            10,
            Math.round(
              50 + (kalshiZScore / (1 + Math.abs(kalshiZScore) * 0.2)) * 25,
            ),
          ),
        );
        const kalshiMarketImplied = Math.min(
          85,
          Math.max(
            20,
        ...
        const edge =
          Math.round((kalshiModelProb - kalshiMarketImplied) * 10) / 10;
        ...
        await ctx.db.insert("props", {
          ...
          projectionSources: [
            ...
            {
              source: "Sharp Model",
              value: km.projection + (pseudoRandom(km.statType) - 0.5) * 3,
            },
          ...
          historicalHitRate: {
            similarLines: Math.round(50 + edge * 0.2),
            sampleSize: 30,
            vsTeam: Math.round(50 + edge * 0.25),
          },
          dataSource: "demo" as const,
          lastUpdated: Date.now(),
    ...
    const txns = [
      {
        ...
        amount: 75,
        sport: "NBA",
        description: "6-pick Flex Play — 5/6 hit",
        timestamp: now - 1 * day,
      },
      {
        platform: "PrizePicks",
        ...
        amount: 125,
        sport: "NBA",
        description: "4-pick Power Play — all hit!",
        timestamp: now - 2 * day,
      },
      {
        platform: "Underdog",
        ...
        amount: -25,
        sport: "NFL",
        description: "3-pick entry — 1/3 hit",
        timestamp: now - 1 * day,
      },
      {
        platform: "Underdog",
        ...
        amount: -10,
        sport: "NBA",
        description: "2-pick entry — 0/2 hit",
        timestamp: now - 3 * day,
      },
      {
        platform: "Sleeper",
        ...
        amount: 60,
        sport: "MLB",
        description: "3-pick Standard — all hit!",
        timestamp: now - 1 * day,
      },
      {
        platform: "Kalshi",
        ...
        amount: 95,
        sport: "NBA",
        description: "Team total OVER — BOS hit 112",
        timestamp: now - 2 * day,
      },
      {
        platform: "DraftKings Pick6",
        ...
        amount: -50,
        sport: "NFL",
        description: "5-pick entry — 2/5 hit",
        timestamp: now - 4 * day,
      },
      {
        platform: "PrizePicks",
        ...
        amount: 40,
        sport: "MLB",
        description: "2-pick Standard — both hit",
        timestamp: now - 3 * day,
      },
      {
        platform: "Kalshi",
      ...
      },
      {
        ...
        amount: 85,
        sport: "NBA",
        description: "4-pick Flex — 4/4 hit!",
        timestamp: now - 2 * day,
      },
      {
        platform: "PrizePicks",
      ...
      },
      {
        ...
        amount: 150,
        sport: "NBA",
        description: "5-pick Power Play — all hit! 🔥",
        timestamp: now - 4 * day,
      },
    ];
