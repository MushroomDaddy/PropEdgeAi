...
export const allProviders = query({
	...
	handler: async (ctx) => {
		...
		const [props, players, games, kalshiMarkets, liveEvents, liveOdds] =
			await Promise.all([
				ctx.db
					.query("props")
					.collect()
		...
		const staleEvents = allLiveEvents.filter(
			(e) =>
				computeRefreshStatus(e.lastUpdated, e.staleAfterMinutes) === "stale",
		).length;
		const freshEvents = allLiveEvents.filter(
			(e) =>
				computeRefreshStatus(e.lastUpdated, e.staleAfterMinutes) === "fresh",
		).length;
		...
		const mergedProviders = statuses.map((s) => {
			...
			if (dbConfig) {
				return {
					...
					status: dbConfig.apiKeyConfigured
						? dbConfig.enabled
							? "active"
							: "inactive"
						: "inactive",
					...
					providerHealth: dbConfig.apiKeyConfigured
						? dbConfig.lastSyncStatus === "success"
							? 90
							: dbConfig.lastSyncStatus === "error"
								? 30
								: 50
						: 0,
					refreshStatus: dbConfig.lastSyncTime
						? computeRefreshStatus(
								dbConfig.lastSyncTime,
								dbConfig.staleAfterMinutes,
							)
						: "never",
				};
			}
		...
		const hasLiveProvider = mergedProviders.some(
			(p) => p.isLive && p.apiKeyConfigured,
		);
