...
function computeValueScore(prop: any): number {
  ...
  const histHitRate = prop.historicalHitRate?.similarLines || prop.hitRate || 50;
...
export const list = query({
  ...
  handler: async (ctx, { sport, platform }) => {
    let q;
    if (sport && platform) {
      q = ctx.db.query("props").withIndex("by_sport_platform", (qb) => qb.eq("sport", sport).eq("platform", platform));
    } else if (sport) {
      q = ctx.db.query("props").withIndex("by_sport", (qb) => qb.eq("sport", sport));
    } else if (platform) {
      q = ctx.db.query("props").withIndex("by_platform", (qb) => qb.eq("platform", platform));
    } else {
      q = ctx.db.query("props");
    }
    const results = await q.collect();
    return results.filter(p => isFinite(p.edge)).map(p => ({
      ...p,
...
export const getTopEdges = query({
  ...
  handler: async (ctx, { limit }) => {
    ...
    const validProps = allProps.filter(p => isFinite(p.edge)).map(p => ({
      ...p,
      valueScore: computeValueScore(p),
    }));
...
export const getTopValuePicks = query({
  ...
  handler: async (ctx, { limit }) => {
    const allProps = await ctx.db.query("props").collect();
    const scored = allProps.filter(p => isFinite(p.edge)).map(p => ({
      ...p,
      valueScore: computeValueScore(p),
    }));
...
export const suggestDiversificationPicks = query({
  args: { currentPickSports: v.array(v.string()), currentPickPlatforms: v.array(v.string()), currentOverUnder: v.array(v.string()) },
  returns: v.array(v.any()),
  handler: async (ctx, { currentPickSports, currentOverUnder }) => {
    const allProps = await ctx.db.query("props").collect();
    const validProps = allProps.filter(p => isFinite(p.edge) && Math.abs(p.edge) > 3);

    // Count current distribution
    const sportCounts: Record<string, number> = {};
    for (const s of currentPickSports) sportCounts[s] = (sportCounts[s] || 0) + 1;
    ...
    const suggestions = validProps
      .map(p => {
        ...
        return { ...p, valueScore: computeValueScore(p), diversityBonus, totalScore: computeValueScore(p) + diversityBonus };
      })
...
export const stats = query({
  ...
  handler: async (ctx) => {
    ...
    const avgEdge = validProps.length > 0
      ? Math.round((validProps.reduce((sum, p) => sum + Math.abs(p.edge), 0) / validProps.length) * 10) / 10
      : 0;
