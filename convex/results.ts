...
export const myResults = query({
  ...
  handler: async (ctx, { sport, platform, status }) => {
    ...
    return results.sort(
      (a, b) => (b.gradedAt || b.pickedAt) - (a.gradedAt || a.pickedAt),
    );
  },
});
  ...
  handler: async ctx => {
    ...
    const avgEdge =
      graded.length > 0
        ...
        : 0;
    const avgCLV =
      graded.filter(r => r.clv !== undefined).length > 0
        ? graded
            .filter(r => r.clv !== undefined)
            .reduce((sum, r) => sum + (r.clv || 0), 0) /
    ...
    return {
      ...
      pending,
      winRate:
        graded.length > 0 ? Math.round((won / (won + lost)) * 1000) / 10 : 0,
...
export const modelPerformance = query({
  ...
  handler: async ctx => {
    ...
    const hitRateByConfidence = Object.entries(confBuckets)
      .map(([bucket, d]) => ({
        bucket,
    ...
    const roiByEdge = Object.entries(edgeBuckets)
      .map(([bucket, d]) => ({
        bucket,
        hitRate: Math.round((d.hits / d.total) * 1000) / 10,
        total: d.total,
    ...
    for (const p of graded) {
      if (!platBuckets[p.platform])
        platBuckets[p.platform] = { total: 0, hits: 0 };
    ...
    for (const p of graded) {
      if (!playerPerf[p.playerName])
        playerPerf[p.playerName] = { total: 0, hits: 0 };
    ...
    return {
      ...
      overallHitRate:
        graded.length > 0
          ? Math.round(
              (graded.filter(p => p.hit).length / graded.length) * 1000,
            ) / 10
          : 0,
      hitRateByConfidence,
      roiByEdge,
