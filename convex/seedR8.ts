...
export const seedResultsAndIntel = mutation({
  ...
  handler: async ctx => {
    ...
    if (users.length === 0 || props.length === 0 || players.length === 0)
      return null;

    // Clear previous R8 data
    for (const r of await ctx.db.query("pickResults").collect())
      await ctx.db.delete(r._id);
    for (const s of await ctx.db.query("propSnapshots").collect())
      await ctx.db.delete(s._id);
    for (const l of await ctx.db.query("playerGameLogs").collect())
      await ctx.db.delete(l._id);
    for (const m of await ctx.db.query("modelPredictions").collect())
    ...
    const statuses: Array<"won" | "lost" | "push" | "void" | "pending"> = [
      "won",
      "lost",
      "push",
      "void",
      ...
      for (let i = 0; i < 60; i++) {
        ...
        const rawActual =
          status === "pending"
            ...
            : status === "won"
              ? prop.overUnder === "over"
                ? prop.line + 2 + Math.random() * 5
                : Math.max(
                    0,
                    prop.line -
                      2 -
                      Math.random() * Math.min(3, prop.line * 0.3),
                  )
              : status === "lost"
                ? prop.overUnder === "over"
                  ? Math.max(
                  ...
                  : prop.line + 1 + Math.random() * 3
                : prop.line; // push
        const actualStat =
          rawActual !== undefined ? Math.max(0, rawActual) : undefined;
        const closingLine =
          status !== "pending"
            ? prop.line + (Math.random() * 2 - 1)
            : undefined;
          ...
          closingLine !== undefined
            ? Math.round(
                (prop.overUnder === "over"
                  ...
                  : prop.line - closingLine) * 10,
              ) / 10
            : undefined;
        const roi =
          status === "won"
            ? Math.round((80 + Math.random() * 40) * 10) / 10
            : status === "lost"
              ? -100
              : 0;
        ...
        await ctx.db.insert("pickResults", {
          ...
          actualStat:
            actualStat !== undefined
              ? Math.round(actualStat * 10) / 10
              : undefined,
          closingLine,
          closingOdds:
            closingLine !== undefined
              ? -110 + Math.round(Math.random() * 20 - 10)
              : undefined,
          clv,
          ev: Math.round(pickEdge * 1.5 * 10) / 10,
          roi: status !== "pending" ? roi : undefined,
          gradedAt:
            status !== "pending" ? now - (daysAgo - 0.5) * DAY : undefined,
          dataSource: "demo",
        });
      }
    } // end for each user
    ...
    for (const prop of props.slice(0, 30)) {
      ...
      const lineJitter = () =>
        Math.round((Math.random() * 1.5 - 0.75) * 10) / 10;
      ...
      await ctx.db.insert("propSnapshots", {
        ...
        marketImpliedProb:
          (prop.marketImpliedProb || 50) + Math.round(Math.random() * 4 - 2),
        odds: -110,
    ...
    for (const player of players) {
      ...
      for (let g = 0; g < 15; g++) {
        ...
        const basePoints = isBball
          ? player.seasonAvg?.points || 20
          : isFootball
            ? 15
            : isBaseball
              ? 1.5
        ...
        await ctx.db.insert("playerGameLogs", {
          ...
          gameDate:
            now -
            (g + 1) * (isBball ? 2 * DAY : isFootball ? 7 * DAY : 1.5 * DAY),
          homeAway: ha,
          points: isBball
            ? Math.round(basePoints + (Math.random() * variance * 2 - variance))
            : isHockey
              ? Math.floor(Math.random() * 3)
              : undefined,
          rebounds: isBball
            ? Math.round(
                (player.seasonAvg?.rebounds || 5) + (Math.random() * 4 - 2),
              )
            : undefined,
          assists: isBball
            ? Math.round(
                (player.seasonAvg?.assists || 4) + (Math.random() * 4 - 2),
              )
            : undefined,
          ...
          fg: isBball
            ? `${Math.floor(5 + Math.random() * 7)}/${Math.floor(12 + Math.random() * 10)}`
            : undefined,
    ...
    for (let i = 0; i < 200; i++) {
          ...
          : modelProb >= 80
            ...
            : modelProb >= 70
              ...
              : modelProb >= 60
                ...
                : "50-60";
      const edgeBucket =
        edge >= 15 ? "15+" : edge >= 10 ? "10-15" : edge >= 5 ? "5-10" : "0-5";
      ...
      const rawPredActual = hit
        ? prop.overUnder === "over"
          ? prop.line + 1 + Math.random() * 5
          : Math.max(
              0,
              prop.line - 1 - Math.random() * Math.min(3, prop.line * 0.3),
            )
        : prop.overUnder === "over"
          ? Math.max(
              0,
