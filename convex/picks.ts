...
export const analyzeCorrelations = query({
  ...
  handler: async ctx => {
    const userId = await getAuthUserId(ctx);
    if (!userId)
      return { warnings: [], suggestions: [], diversificationScore: 0 };
    ...
    if (picks.length < 2)
      return { warnings: [], suggestions: [], diversificationScore: 100 };
    ...
    for (const [, group] of Object.entries(gameGroups)) {
      if (group.length >= 2) {
        ...
        if (teams.length === 1) {
          warnings.push(
            `🔗 ${names} are teammates — their stats are positively correlated. Both hitting is more likely (or both missing).`,
          );
          suggestions.push(
            `💡 Consider: If ${group[0].playerName} has a big game, ${group[1]?.playerName || "teammates"} may benefit too. Good for upside plays.`,
          );
        } else {
          warnings.push(
            `⚔️ ${names} are in the same game — game flow affects both. If one team blows out, the other's players may see reduced minutes.`,
          );
        }
      }
    }
      ...
      for (let j = i + 1; j < picks.length; j++) {
        if (picks[i].playerName === picks[j].playerName) {
          ...
          if (correlated?.includes(picks[j].statType)) {
            warnings.push(
              `📊 ${picks[i].playerName}: ${picks[i].statType} and ${picks[j].statType} are statistically correlated (r > 0.7). Picking the same direction on both adds risk.`,
            );
          }
        }
      }
    ...
    if (overs > 0 && unders === 0 && picks.length >= 3) {
      suggestions.push(
        `📈 All ${overs} picks are OVER — consider adding an under for balance. All-over entries are higher variance.`,
      );
    } else if (unders > 0 && overs === 0 && picks.length >= 3) {
      suggestions.push(
        `📉 All ${unders} picks are UNDER — consider mixing in an over for diversification.`,
      );
    }
    ...
    if (sportEntries.length === 1 && picks.length >= 4) {
      suggestions.push(
        `🏟️ All picks from ${sportEntries[0][0]} — diversifying across sports reduces correlated loss risk.`,
      );
    }
    ...
    for (let i = 0; i < picks.length; i++) {
      for (let j = i + 1; j < picks.length; j++) {
        if (picks[i].gameId && picks[i].gameId === picks[j].gameId) {
          ...
          if (iTeam !== jTeam) {
            if (
              picks[i].statType.includes("Points") &&
              picks[j].statType.includes("Points") &&
              picks[i].overUnder === "over" &&
              picks[j].overUnder === "over"
            ) {
              warnings.push(
                `⚠️ ${picks[i].playerName} & ${picks[j].playerName} are opposing players — both OVER on scoring is a high-pace game bet.`,
              );
            }
          }
        }
    ...
    const overUnderBalance =
      Math.min(overs, unders) / Math.max(overs, unders, 1);
    ...
    const diversificationScore = Math.round(
      (gameDiv * 0.4 + sportDiv * 0.3 + balanceScore * 0.3) * 100,
    );
...
export const generateQuickPack = query({
  ...
  handler: async (ctx, { packType }) => {
    ...
    switch (packType) {
      case "bestEdge4": {
        const sorted = [...validProps].sort(
          (a, b) => Math.abs(b.edge) - Math.abs(a.edge),
        );
        const picks = pickUnique(sorted, 4);
        return {
          name: "🎯 Best Edge 4-Pack",
          description: "Top 4 highest absolute edge plays",
          picks,
          avgEdge: avg(picks.map(p => Math.abs(p.edge))),
      ...
      case "highConfidence3": {
        ...
        return {
          name: "🛡️ High Confidence 3-Pack",
          description: "Safest picks with highest confidence scores",
          picks,
          avgEdge: avg(picks.map(p => Math.abs(p.edge))),
          avgConfidence: avg(picks.map(p => p.confidence)),
          sports: [...new Set(picks.map(p => p.sport))],
      ...
      case "balancedMix": {
        const overs = [...validProps]
          .filter(p => p.edge > 5)
          .sort((a, b) => b.edge - a.edge);
        const unders = [...validProps]
          .filter(p => p.edge < -5)
        ...
        return {
          name: "⚖️ Balanced EV Mix",
          description: "2 overs + 2 unders for balanced exposure",
          picks,
          avgEdge: avg(picks.map(p => Math.abs(p.edge))),
          avgConfidence: avg(picks.map(p => p.confidence)),
          sports: [...new Set(picks.map(p => p.sport))],
      ...
      case "kalshiBest": {
        const kalshi = validProps.filter(
          p => p.platform === "Kalshi" || p.isKalshiMarket,
        );
        const sorted = [...kalshi].sort(
          (a, b) => Math.abs(b.edge) - Math.abs(a.edge),
        );
        const picks = pickUnique(sorted, 4);
        return {
          name: "💹 Kalshi Best Markets",
          description: "Highest edge Kalshi binary markets",
          picks,
          avgEdge: avg(picks.map(p => Math.abs(p.edge))),
          avgConfidence: avg(picks.map(p => p.confidence)),
          sports: [...new Set(picks.map(p => p.sport))],
      ...
      case "underdogFlex": {
        const udProps = validProps.filter(
          p => p.platform === "Underdog" || Math.abs(p.edge) > 8,
        );
        const sorted = [...udProps].sort(
          (a, b) => Math.abs(b.edge) - Math.abs(a.edge),
        );
        const picks = pickUnique(sorted, 6);
        return {
          name: "🐕 Underdog 6-Pick Flex",
          description: "Optimized for Underdog Flex Play (5/6 or 6/6 win)",
          picks,
          avgEdge: avg(picks.map(p => Math.abs(p.edge))),
          avgConfidence: avg(picks.map(p => p.confidence)),
          sports: [...new Set(picks.map(p => p.sport))],
      ...
      case "prizePicksPower": {
        const ppProps = validProps
          .filter(p => p.edge > 3)
          .sort((a, b) => b.confidence - a.confidence);
        const picks = pickUnique(ppProps, 4);
        return {
          name: "⚡ PrizePicks Power Play",
          description: "4-pick power play — all must hit for 10x",
          picks,
          avgEdge: avg(picks.map(p => Math.abs(p.edge))),
          avgConfidence: avg(picks.map(p => p.confidence)),
          sports: [...new Set(picks.map(p => p.sport))],
      ...
      case "topValue5": {
        const scored = validProps.map(p => {
          ...
          const cf = cons
            ? p.edge > 0
              ? (cons.numOverLine / cons.numSources) * 25
              : ((cons.numSources - cons.numOverLine) / cons.numSources) * 25
            : 10;
          const hr = Math.min(
            20,
            ((p.historicalHitRate?.similarLines || p.hitRate || 50) / 100) * 20,
          );
          const br = Math.min(15, ((100 - (p.bustRisk ?? 40)) / 100) * 15);
          return {
            ...p,
            valueScore: Math.round(Math.min(100, absEdge + cf + hr + br)),
          };
        });
        scored.sort((a, b) => b.valueScore - a.valueScore);
        const picks = pickUnique(scored, 5);
        return {
          name: "💎 Top Value 5-Pack",
          description: "Highest composite Value Score picks",
          picks,
          avgEdge: avg(picks.map(p => Math.abs(p.edge))),
          avgConfidence: avg(picks.map(p => p.confidence)),
          sports: [...new Set(picks.map(p => p.sport))],
...
export const pickStats = query({
  ...
  handler: async ctx => {
    ...
    const winRate =
      decided > 0 ? Math.round((wonPicks / decided) * 1000) / 10 : 0;
